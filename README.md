...
1. **Полноценная галерея** — свайп-слайдер с инерцией, lightbox по клику, zoom-in на тап
2. **Свой HEX для цветов из админки** — вместо хардкода в `result_modifier.php` брать из файла или справочника
3. **Persistent корзина в хедере** — сейчас обновляется только в ответе AJAX, но DOM-счётчик в шапке не трогается (нужна интеграция с шаблоном сайта)
4. **Skeleton-loader** — при первой загрузке показать placeholder вместо рывка
5. **Тесты** — PHPUnit для `result_modifier` (SKU-матрица) и Jest для класса `ProductCard`
6. **Оптимизация изображений** — автоматическая WebP-конвертация через хук на `OnFileSave`
7. **Composite-сайт** — настроить для ускорения первой загрузки на мобильных
8. **«Сообщить о поступлении»** — когда товар out-of-stock, кнопка меняется на форму подписки
9. **SEO-разметка** — Schema.org `Product` с offers/availability/price
10. **Accessibility** — `aria-pressed` на свотчах/чипах, `role="region"` для карточки, keyboard navigation
...
| Дизайн + план | ~0.5 ч |
| Docker | ~0.5 ч |
| Шаблон компонента (result_modifier, template, css, js) | ~2 ч |
| AJAX-эндпоинт | ~0.5 ч |
| Seeder | ~0.5 ч |
